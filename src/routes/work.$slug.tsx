...
Every product could have customized stages. Overall there were 35+ types of notifications on the platform.\u00a0`}
      ...
      <section className={`mx-auto max-w-[1080px] px-6 md:px-10 ${c.slug === "solace" ? "pt-[100px] pb-0" : "py-24"}`}>
        <h2 className={`font-serif mb-8 ${c.slug === "solace" ? "text-3xl mt-12 font-medium" : "text-4xl md:text-5xl font-normal"}`}>{(c.slug === "fjord2") ? "Initial Design" : (c.slug === "atlas" || c.slug === "atlas2") ? "User Flow" : (c.slug === "fjord") ? "Solution" : "Brainstorming Session With Product Team"}</h2>
      ...
      {(c.slug !== "fjord2") && (
        <section id="testing" className={`mx-auto max-w-[1080px] px-6 md:px-10 scroll-mt-24 ${c.slug === "solace" ? "pt-24" : "py-24"}`}>
          ...
          <ul className="space-y-6 text-lg leading-relaxed text-foreground/85">
            ...
            <li className="flex gap-4">
              <span className="mt-3 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/60" aria-hidden />
              <span>{c.slug === "fjord" ? "\u00a0the objective of the test was to identify How easily users are able to understand usability and how intuitive it is for users to complete the given task." : "Usability Test Findings -\u00a0\n1. 2 out of the 4 participants felt the need of adding the label are detailed sketch page.\n2. 3 out of the 4 participants wanted to save button if they wanted to leave it halfway and need to navigate to the part of the application.\n2. 3 out of the 4 participants were confused with the details drawer not opening at Bill of Material page.\u00a0\nAll participants felt that the design is serving the purpose and addressing the pain points they had."}</span>
            </li>
            <li className="flex gap-4">
              <span className="mt-3 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/60" aria-hidden />
