...
import sketchImg1 from "@/assets/fjord-component-1.png.asset.json";
...
\u00a03.Users need only selective set of notifications which are important for their job responsibilities\u00a0`}
          ...
          <div>
            <h2 className="font-serif text-3xl mb-6 font-medium">{(c.slug === "fjord2") ? "Intoduction" : (c.slug === "fjord" ? "The Problem" : "Goal")}</h2>
...
If AE’s want to build a custom audience to for the clients would require to send a request\x03to the data team and the turn around time was at least a week as the team is small. \x03AE’s had to wait for getting their coustom audience for the campaign.`}</p>
      ...
      {/* Strategy */}
      <section id="strategy" className="mx-auto max-w-[1080px] px-6 md:px-10 py-24 scroll-mt-24" style={{ display: (c.slug === "atlas" || c.slug === "atlas2" || c.slug === "fjord") ? "none" : undefined }}>
...
Every product could have customized stages. Overall there were 35+ types of notifications on the platform.\u00a0`}
        ...
        <p className="text-lg leading-relaxed text-foreground/85 mb-4">
          {(c.slug === "fjord2") 
            ...
            : "This tool needed a custom components to be designed as which was mainly the two parts\u00a0"
          }
        </p>
        {(c.slug !== "fjord2") && (c.slug !== "atlas" && c.slug !== "atlas2") && (
          ...
          <div className="flex flex-row gap-4 overflow-x-auto pb-2">
            {(c.slug === "fjord" ? [fjordComponentLeft, sketchImg2] : [sketchImg1, sketchImg2]).map((s, i) => (
              <button
                ...
                className="group block flex-shrink-0 w-[48.1%] min-w-[393px] overflow-hidden rounded-sm bg-secondary p-2 transition-colors cursor-zoom-in"
              >
                <img
                  src={s.url}
      ...
      <section className={`mx-auto max-w-[1080px] px-6 md:px-10 ${c.slug === "solace" ? "pt-8 pb-0" : "py-24"}`}>
        <h2 className="font-serif text-4xl mb-8 font-medium md:text-3xl">{(c.slug === "fjord2") ? "Initial Design" : (c.slug === "atlas" || c.slug === "atlas2") ? "User Flow" : (c.slug === "fjord") ? "Brainstorming" : "Brainstorming Session With Product Team"}</h2>
