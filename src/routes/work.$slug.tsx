      ...
      <div className={`grid grid-cols-2 gap-8 md:gap-12 ${c.outcome.length >= 5 ? "md:grid-cols-5" : "md:grid-cols-4"}`}>
        {c.outcome.map((o) => (
          <div key={o.label} className="border-t border-border pt-6">
            {o.value && (
              <p className="display text-4xl md:text-5xl text-accent mb-3 whitespace-pre-line">
        ...
        <section className="mx-auto max-w-[1080px] px-6 md:px-10 py-24">
              ...
              <div className="relative overflow-hidden rounded-sm border-l border-[#0068FF] dark:border-primary/70 pl-8 md:pl-12 max-w-3xl mx-auto my-[50px] py-10 pr-8 md:pr-12">
                ...
                <blockquote className="relative z-10 text-lg md:text-xl leading-relaxed text-left whitespace-pre-line">
                  {"Refer to the Survey Results section, you will find that we asked a question to rate the current experience of the affiliate site. The survey results surprised us that, the most respondents were satisfied, and some answered as very satisfied. During a session with one of the affiliates, they said, "}<strong><em>\"We like it as it is, we don't want it to change.</em></strong>{"\" Because the site is very old, the users are so used to it ; even with the workarounds. Change is very hard for them after learning to operate with all the workarounds turned into user habits built over years of using the website.\n\n\n"}<span className="block font-serif text-3xl mb-6 font-medium">The Real Problem</span>{"\nThe hardest constraint wasn't technical. Users were satisfied because they'd normalized the workarounds, so the risk wasn't a worse tool ; it was rejection of a better one.\n\nThe real question wasn't\u00a0\n“How do we make the product better?”\n\nIt was:\n“How do we make the product better without breaking the behavior that keeps the business running?”\n\nSo, I designed for the flat adoption over dramatic change.\n"}
                </blockquote>
              </div>
          ...
          {c.slug !== "solace" && c.slug !== "fjord" && (
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                { label: "Error-free rate", value: "85%" },
      ...
      <section id={topOutcomeSlugs.has(c.slug) ? "reflection" : "impact"} className="mx-auto max-w-[1080px] px-6 md:px-10 pt-24 pb-0 scroll-mt-24">
        ...
        <p className="text-lg leading-relaxed text-foreground/85 mb-20">
          {c.slug === "affiliate-platform-redesign" 
            ...
            : (c.slug === "atlas" || c.slug === "atlas2")
            ? <>
                The design did what it set out to do: users could finally control notifications at the level that matched their work{"\u00A0"} company, product, and lifecycle stage and the critical alerts stopped drowning in the noise. That fed directly into fewer missed deadlines and a better client experience, which mattered for retention. But the harder lesson was that giving users control is not the same as reducing their burden. Preferences across that many levels can quietly become their own kind of overwhelm a settings screen as noisy as the feed it was meant to fix. So rather than hand users a blank grid of toggles, I leaned on role-based <strong>Notification Templates:</strong> each user group starts with a default set mapped to their job and validated against my research. Template creation lives on web; on mobile, users inherit their role's template and fine-tune it if they want. Most people get the right notifications without ever opening settings{"\u00A0"} control is there for the ones who need it.
              </>
