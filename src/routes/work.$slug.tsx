    ...
    <SiteLayout>
      ...
      <section className="mx-auto max-w-[1080px] px-6 md:px-10 pb-16">
        <div className="overflow-hidden rounded-sm">
          <img src={c.cover} alt={c.title} width={1400} height={1000} className={`h-auto ${(c.slug === "atlas" || c.slug === "atlas2") ? "w-[35%] mx-auto" : "w-full"}`} />
        </div>
      </section>
...
\u00a03.Users need only selective set of notifications which are important for their job responsibilities\u00a0`}
      ...
      {/* Strategy */}
      <section id="strategy" className="mx-auto max-w-[1080px] px-6 md:px-10 py-24 scroll-mt-24" style={{ display: (c.slug === "atlas" || c.slug === "atlas2") ? "none" : undefined }}>
...
Every product could have customized stages. Overall there were 35+ types of notifications on the platform.\u00a0`}
        ...
        <h3 className="font-serif text-3xl mt-12 mb-6 font-medium">
          {(c.slug === "fjord2") ? "Sitemap" : (c.slug === "atlas" || c.slug === "atlas2") ? "Mid-Fidelity Wireframes" : "Initial sketch"}
        </h3>
      ...
      {/* Brainstorming */}
      <section className={`mx-auto max-w-[1080px] px-6 md:px-10 ${c.slug === "solace" ? "pt-24 pb-0" : "py-24"}`}>
        <h2 className="font-serif text-4xl mb-8 font-medium md:text-3xl mt-0">{(c.slug === "fjord2") ? "Initial Design" : (c.slug === "atlas" || c.slug === "atlas2") ? "User Flow" : "Brainstorming Session With Product Team"}</h2>
